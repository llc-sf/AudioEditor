...
package dev.android.player.widget.link;
...
public class LinkBuilder {
    ...
    public static LinkBuilder from(Context context) {
        return new LinkBuilder(TYPE_TEXT)
                .setContext(context);
    }
...
//        Logger.d(TAG,"pattern: "+link.getText() + "");
//        Logger.d(TAG,text + "");
//        Logger.d(TAG,"S: " + s);
        ...
        while (matcher.find()) {
            ...
            if (start >= 0) {
                if (!TextUtils.isEmpty(link.getRealText())) {
                    text = TextUtils.replace(s, new String[]{link.getText()}, new CharSequence[]{link.getRealText()});
                    spannable = SpannableString.valueOf(text);
                    int end = start + link.getRealText().length();
                    // add link to the spannable text
                    applyLink(link, new Range(start, end), spannable);
                } else {
