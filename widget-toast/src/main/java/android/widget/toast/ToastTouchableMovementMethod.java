...
public class ToastTouchableMovementMethod extends LinkMovementMethod {

    // TouchableSpan to handle the clicks
    private ToastTouchBaseSpan mPressedSpan;

    public ToastTouchBaseSpan getPressedSpan() {
        return mPressedSpan;
    }
    ...
    public boolean onTouchEvent(final TextView textView, final Spannable spannable, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            ToastTouchBaseSpan touchedSpan = getPressedSpan(textView, spannable, event);
     ...
     */
    private ToastTouchBaseSpan getPressedSpan(TextView widget, Spannable spannable, MotionEvent event) {
        ...
        if (off != end && off != end - 1) {
            ToastTouchBaseSpan[] link = spannable.getSpans(off, off, ToastTouchBaseSpan.class);

            if (link.length > 0)
                return link[0];
        } else {
            // 张一苇的宏观茶室 07-16 17：38的帖子 链接在一行文字的最后，点击蓝色链接无效，故添加这一段
            ToastTouchBaseSpan[] link = null;
            try {
                link = spannable.getSpans(off, off, ToastTouchBaseSpan.class);
    ...
    private static ToastTouchableMovementMethod sInstance;
    ...
    public static MovementMethod getInstance() {
        if (sInstance == null)
            sInstance = new ToastTouchableMovementMethod();
