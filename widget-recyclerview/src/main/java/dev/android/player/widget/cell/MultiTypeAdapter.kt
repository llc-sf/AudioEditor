...
open class MultiTypeAdapter @JvmOverloads constructor(
         ...
         * @since v2.4.1
         */
        open var items: List<Any> = emptyList(), open val initialCapacity: Int = 0, open var types: Types = MutableTypes(initialCapacity)) : RecyclerView.Adapter<ViewHolder>() {
    ...
    private fun getOutDelegateByViewHolder(holder: ViewHolder): ItemViewDelegate<Any, ViewHolder> {
        @Suppress("UNCHECKED_CAST") return types.getType<Any>(holder.itemViewType).delegate as ItemViewDelegate<Any, ViewHolder>
    }
    ...
    open fun removeData(data: Any?) {
        val position: Int = items.indexOf(data)
        if (position >= 0) {
            removeData(position)
        }
    }
    ...
    open fun moveData(fromPosition: Int, toPosition: Int) {
        if (fromPosition < 0 || fromPosition >= items.size || toPosition < 0 || toPosition >= items.size || fromPosition == toPosition) {
            return
        }
        val data = items[fromPosition]
        (items as ArrayList).removeAt(fromPosition)
        (items as ArrayList).add(toPosition, data)
        notifyDataSetChanged()
    }

    open fun notifyItem(data: Any) {
        val position: Int = items.indexOf(data)
        if (position != -1) {
