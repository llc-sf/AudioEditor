...
public class DragSortRecycler extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    private int mLastHeaderPos = -1;

    private int getLastHeaderPos(RecyclerView rv) {
        int foundHeaderPos = -1;
        ...
        return foundHeaderPos;
    }

    private int getVisuableHeaderBottom(RecyclerView rv, int headerPos) {
        int bottom = 0;
        RecyclerView.LayoutManager manager = rv.getLayoutManager();
        if (manager != null) {
            do {
                View view = manager.findViewByPosition(headerPos);
                if (view != null) {
                    bottom = view.getScrollY() + view.getHeight();
                    break;
                } else {
                    headerPos--;
                }
    ...
    public void onTouchEvent(RecyclerView rv, MotionEvent e) {
        ...
        if (floatingItem != null) {
            ...
            if (mLastHeaderPos < 0) {
                mLastHeaderPos = getLastHeaderPos(rv);
            }
            if (mLastHeaderPos >= 0) {
                offsetY = getVisuableHeaderBottom(rv, mLastHeaderPos);
            }

            if (floatingItemBounds.top < offsetY - floatingItemStatingBounds.height() / 2) //Allow half the view out the top
                floatingItemBounds.top = offsetY - floatingItemStatingBounds.height() / 2;
    ...
    protected boolean canDragOver(int position) {
        return position > mLastHeaderPos;
    }
