 ...
 * 毛刺效果 带阴影
 */
open class WaveformSeekBar7 @JvmOverloads constructor(
   ...
   private var waveHeightScale: Float = 0.5f
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val smoothness = 0.9f // 控制曲线平滑度的因子，范围[0, 1]
        ...
        for (i in 1 until upperPoints.size) {
            ...
            val midX = (prevX + x) / 2
            val midY = (prevY + y) / 2
            val controlX1 = prevX + (midX - prevX) * smoothness
            val controlY1 = prevY + (midY - prevY) * smoothness
            val controlX2 = x - (x - midX) * smoothness
            val controlY2 = y - (y - midY) * smoothness
            path.cubicTo(controlX1, controlY1, controlX2, controlY2, x, y) // 三次贝塞尔曲线
            prevX = x
            prevY = y
        }
        ...
        for (i in upperPoints.size - 2 downTo 0) {
            val (x, origY) = upperPoints[i]
            val y = 2 * centerY - origY // 镜像Y坐标
            val midX = (prevX + x) / 2
            val midY = (prevY + y) / 2
            val controlX1 = prevX + (midX - prevX) * smoothness
            val controlY1 = prevY + (midY - prevY) * smoothness
